...
export function AuditPage() {
  ...
  return <WorkspacePage title="Audit Log" actions={<button disabled={action.pending} onClick={() => action.run(() => downloadCsv(token, search))}>{action.pending ? 'Exporting…' : 'Export CSV'}</button>}><label>Search events<input aria-label="Search events" value={search} onChange={event => setSearch(event.target.value)} /></label>
    {action.feedback}
