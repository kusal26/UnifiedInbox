...
public sealed class AuthenticationService(InboxDbContext db, IPasswordHasher<User> passwords, ITokenIssuer tokens, ICurrentTenant currentTenant, IMailSender mail, ITenantExecutionScope? executionScope = null) : IAuthService
{
    private ITenantExecutionScope Scope { get; } = executionScope ?? new TenantExecutionScope(db);
