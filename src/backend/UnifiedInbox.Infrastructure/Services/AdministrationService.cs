...
public sealed class WebhookService(InboxDbContext db, UnifiedInbox.Application.Tenancy.ITenantExecutionScope? executionScope = null) : IWebhookService
{
    private UnifiedInbox.Application.Tenancy.ITenantExecutionScope Scope { get; } = executionScope ?? new TenantExecutionScope(db);
