...
public sealed class ConversationsController(IInboxService inbox) : ControllerBase
{
    ...
    [HttpPost("{id:guid}/messages")] public async Task<IActionResult> Message(Guid id, MessageRequest request, CancellationToken token) { if (!Request.Headers.TryGetValue("Idempotency-Key", out var key) || string.IsNullOrWhiteSpace(key)) return BadRequest(Problem(title: "Idempotency-Key is required", statusCode: 400)); return await inbox.SendAsync(id, request.Body, key.ToString(), token) is { } item ? Accepted(item) : NotFound(); }
...
public sealed record NoteRequest(string Body); public sealed record MessageRequest(string Body); public sealed record StatusRequest(ConversationStatus Status); public sealed record ReadRequest(long ThroughSequence); public sealed record CustomerNotesRequest(string? Notes);
