...
public sealed record NoteRequest(string Body); public sealed record MessageRequest(string Body, string? TemplateName = null, IReadOnlyList<Guid>? AttachmentIds = null); public sealed record StatusRequest(ConversationStatus Status); public sealed record ReadRequest(long ThroughSequence); public sealed record CustomerNotesRequest(string? Notes);
