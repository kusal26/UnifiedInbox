...
public sealed class AttachmentsController(IAttachmentService attachments) : ControllerBase
{
    ...
    [HttpGet("{id:guid}/download")] public async Task<IActionResult> Download(Guid id, CancellationToken token) => await attachments.DownloadAsync(id, token) is { } file ? Ok(new { downloadUrl = file.DownloadUrl, contentType = file.ContentType, fileName = file.FileName, expiresAt = file.ExpiresAt }) : NotFound();
}
