...
public sealed class WebhooksController(IWebhookService webhooks, WhatsAppSignatureValidator signatures, IConfiguration configuration) : ControllerBase
{
    ...
    {
        using var memory = new MemoryStream(); await Request.Body.CopyToAsync(memory, token); var body = memory.ToArray();
        var secret = configuration["WhatsApp:AppSecret"] ?? "";
        if (string.IsNullOrWhiteSpace(secret) || !signatures.IsValid(body, Request.Headers["X-Hub-Signature-256"], secret)) return Unauthorized();
        using var json = JsonDocument.Parse(body);
        var eventId = FindEventId(json.RootElement) ?? Convert.ToHexString(System.Security.Cryptography.SHA256.HashData(body));
        // Route by metadata.phone_number_id; tenant/channel ids are never accepted from input.
        var assetId = FindPhoneNumberId(json.RootElement);
        if (string.IsNullOrWhiteSpace(assetId)) return NotFound();
        return await webhooks.PersistByAssetAsync(assetId, eventId, body, token) ? Ok(new { received = true }) : NotFound();
    }
    ...
    private static string? FindPhoneNumberId(JsonElement root)
    {
        try { return root.GetProperty("entry")[0].GetProperty("changes")[0].GetProperty("value").GetProperty("metadata").GetProperty("phone_number_id").GetString(); }
