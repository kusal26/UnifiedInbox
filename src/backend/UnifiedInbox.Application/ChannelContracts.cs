...
/// <summary>Public, browser-safe Embedded Signup bootstrap. Contains no secrets: the raw state/nonce
/// are single-use handshake values and the provider values (App id, configuration id, Graph version,
/// Embedded Signup version) are public by design.</summary>
public sealed record ConnectionAttemptInfo(Guid AttemptId, string State, string Nonce, string MetaAppId, string ConfigurationId, string GraphVersion, string EmbeddedSignupVersion, DateTimeOffset ExpiresAt);
...
public interface IWhatsAppGraphClient
{
    ...
    /// <summary>Lists the phone numbers owned by a WhatsApp Business Account (WABA). Used to prove
    /// the signup-returned phone id actually belongs to the granted WABA before subscribing it.</summary>
    Task<IReadOnlyList<GraphPhoneNumber>> GetPhoneNumbersAsync(string businessId, string accessToken, CancellationToken cancellationToken);
...
public interface IChannelService
{
    ...
    Task<ChannelSummary> CompleteConnectAsync(string state, string nonce, string code, string phoneNumberId, string businessId, string displayName, CancellationToken cancellationToken);
